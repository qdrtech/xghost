...
#
# One fact this run cannot read: hyprctl answers only inside a running Hyprland
# session, and there is no session during an installation. This run therefore
# records MACHINE_MONITOR_COUNT=unknown, and the monitor layout the theme step
# renders is the 'default' fragment of the bundle, which lets Hyprland lay the
# displays out itself. Detection names every source it could not read on
# standard error, so the reader sees that here rather than guessing at it later.
#
...
if [ "$INSTALL_DRY_RUN" = yes ]; then
	...
	install_would "record MACHINE_MONITOR_COUNT=unknown, because no Hyprland session is running yet"
	exit 0
fi
